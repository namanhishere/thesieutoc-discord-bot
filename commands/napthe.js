...
module.exports.run = async (client, message, args) => {
    ...
    database.query(`SELECT * FROM serverinfomation WHERE serverID = ${message.guild.id}`, function (error, data) {
        ...
        if (data.length === 0) {
            ...
            if (!args[0] || !args[1] || !args[2] || !args[3]) return message.reply("Vui lòng nhập đầy đủ các thông tin. Câu lệnh sử dụng là ```" + data[0].prefix + "napthe [Tên nhà mang] [Mệnh giá.] [Số Thẻ] [Số Seri]```")
