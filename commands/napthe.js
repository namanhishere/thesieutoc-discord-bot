...
module.exports.run = async (client, message, args) => {
    ...
    database.query(`SELECT * FROM serverinfomation WHERE serverID = ${message.guild.id}`, function (error, data) {
        ...
        if (data.length === 0) {
            ...
            if(isNaN(parseFloat(args[2]))||isNaN(parseFloat(args[3]))) return message.reply("Thẻ nạp phải ở dạng số, vui lòng kiểm tra lại cú pháp```" + data[0].prefix + "napthe [Tên nhà mang] [Mệnh giá.] [Số Seri] [Số thẻ]```")
            let times = Date.now()
